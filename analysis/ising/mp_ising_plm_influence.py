"""
mp_ising_plm_influence.py

2D scatter: per-MP PLM coupling strength vs majority alignment.

  X  c_i = ⟨σ_i · γ⟩          coalition/opposition alignment (raw correlation)
  Y  mean_j |J_ij|             average PLM coupling to all other MPs

Quadrants
  top-right  : coalition enforcers — high coupling, vote with majority
  top-left   : opposition drivers  — high coupling, vote against majority
  top-centre : cross-aisle bridges — high coupling, near-zero alignment
  bottom     : peripheral / unremarkable

Usage
-----
  python analysis/ising/mp_ising_plm_influence.py
  python analysis/ising/mp_ising_plm_influence.py light
"""
...
def fit_plm_coupling(S):
    """Return mean |J_ij| per MP (averaged over all j≠i)."""
    N = S.shape[0]
...
for period_key, lbl in PERIODS:
    ...
    coupling  = fit_plm_coupling(S)

    print(f"  computing alignment…", flush=True)
    alignment = compute_alignment(S)
    ...
    print(f"  done. coupling range: [{coupling.min():.4f}, {coupling.max():.4f}]", flush=True)
...
for idx, (ax, (lbl, df)) in enumerate(zip(axes.flat, period_dfs)):
    ...
    for party, grp in df.groupby("party"):
        ...
        ax.scatter(grp["alignment"], grp["coupling"],
                   c=color, s=18, alpha=0.70, linewidths=0, zorder=3)
    ...
    if ax_col == 0:
        ax.set_ylabel("← Less influence beyond party line  —  More →",
                      color=T["sub"], fontsize=12)
...
fig.text(0.5, 0.997,
         "Parliamentary coupling strength vs majority alignment · Bundestag 2005–2029",
         ha="center", va="top", color=T["text"], fontsize=20, fontweight="bold")
fig.text(0.5, 0.968,
         "Y: voting influence beyond party line — mean |J_ij| from regularised PLM inverse Ising  ·  "
...
out = IMG_DIR / "mp_ising_plm_influence.png"
...
def plot_single_period(lbl: str, df: pd.DataFrame, period_key: str) -> None:
    out_dir = BASE_DIR / "output" / period_key
    out_path = out_dir / f"plm_influence_{_theme}.png"
    ...
    for party, grp in df.groupby("party"):
        ...
        ax.scatter(grp["alignment"], grp["coupling"],
                   c=color, s=22, alpha=0.72, linewidths=0, zorder=3)
    ...
    if chancellor_name:
        ...
        if not row.empty:
            ...
            cy = float(row["coupling"].iloc[0])
            ch_color = party_color.get(row["party"].iloc[0], "#888888")
            ax.scatter(cx, cy, s=160, facecolors="none",
                       edgecolors=hl_color, linewidths=2.0, zorder=6)
            ax.scatter(cx, cy, s=70, c=ch_color, linewidths=0, zorder=7)
            t = ax.text(cx, cy, chancellor_name,
    ...
    for fv_name in FRAKTIONSVORSITZENDE.get(lbl, []):
        ...
        fy = float(row["coupling"].iloc[0])
        fv_color = party_color.get(row["party"].iloc[0], "#888888")
        ax.scatter(fx, fy, s=90, facecolors="none",
                   edgecolors=hl_color, linewidths=1.4, zorder=6, alpha=0.85)
        ax.scatter(fx, fy, s=42, c=fv_color, linewidths=0, zorder=7)
        t = ax.text(fx, fy, fv_name.split()[-1],
                    fontsize=10, color=hl_color, va="center", ha="center", zorder=8)
        hl_texts.append(t); hl_xs.append(fx); hl_ys.append(fy)
    ...
    if hl_texts:
        ...
        for t in hl_texts:
            t.set_path_effects([pe.withStroke(linewidth=2.5, foreground=hl_stroke)])
    ...
    handles = [
        plt.Line2D([0], [0], marker="o", color="none",
                   markerfacecolor=parties_seen.get(p, party_color.get(p, "#888")),
    ...
    ax.set_title(
        f"Parliamentary coupling vs majority alignment  ·  Bundestag {lbl}",
        color=T["text"], fontsize=14, fontweight="bold", loc="left", pad=10)
    ax.text(0, -0.10,
            "Y: mean |J_ij| from regularised PLM inverse Ising  ·  "
            "X: ⟨σ_i · γ⟩ correlation with chamber majority  ·  "
...
for (period_key, lbl), (lbl2, df) in zip(PERIODS, period_dfs):
    plot_single_period(lbl, df, period_key)
