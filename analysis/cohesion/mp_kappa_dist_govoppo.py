"""
mp_kappa_dist_govoppo.py

2×3 grid: one subplot per Bundestag period.
Each subplot shows the κ distribution for government–opposition pairs only.

Usage:
    python analysis/mp_kappa_dist_govoppo.py        # dark
    python analysis/mp_kappa_dist_govoppo.py light
"""
...
LINE_C  = "#2a2a2a" if LIGHT_MODE else "white"
...
for ax, (pk, lbl, coal_key) in zip(axes.flat, PERIODS):
    ...
    # Cross-bloc: one from coalition, one from opposition
    cross_e = edges[edges["pa_coal"] != edges["pb_coal"]]["weight"].values

    kde = gaussian_kde(cross_e, bw_method=0.06)(x)
    kde = kde / kde.max()

    ax.set_facecolor(BG)

    ax.fill_between(x, kde, alpha=0.12 if LIGHT_MODE else 0.15, color=LINE_C)
    ax.plot(x, kde, color=LINE_C, lw=1.8)

    # κ = 0 reference
    ax.axvline(0, color=SUBTEXT, lw=0.8, ls="--", alpha=0.4)

    # Mean tick
    mean_val = cross_e.mean()
    ax.axvline(mean_val, color=SUBTEXT, lw=1.0, alpha=0.7)
    ax.text(mean_val + 0.02, 1.08, f"μ={mean_val:+.2f}",
            transform=ax.get_xaxis_transform(), color=SUBTEXT,
...
fig.text(0.5, 0.960,
         "Kernel density estimate of pairwise Cohen's κ for all government–opposition MP pairs, peak-normalised.  "
         "Vertical line = mean κ.",
         ha="center", va="top", color=SUBTEXT, fontsize=9)

plt.tight_layout(rect=[0, 0, 1, 0.955])
plt.subplots_adjust(hspace=0.38, wspace=0.08)

out = IMG_DIR / "kappa_dist_govoppo.png"
